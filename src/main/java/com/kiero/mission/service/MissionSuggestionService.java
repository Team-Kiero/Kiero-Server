...
public class MissionSuggestionService {
    ...
    // [수정] 기존 내용을 유지하되, 마감일 설정 규칙 부분만 '달력 참조' 방식으로 변경
    private static final String MISSION_SUGGESTION_PROMPT = """
              ...
              6. 최대 10개의 미션을 추천하세요.
              7. 하나의 할 일은 하나의 미션으로 만드세요 (중요)
                 - 예: 독후감 3편 써오기는 1개 미션으로 만드세요
                 - 분리하지 마세요: 독후감 1편, 독후감 2편, 독후감 3편으로 나누지 마세요
              
              8. 마감일(dueAt) 설정 규칙 (매우 중요):
                 **AI는 날짜를 직접 계산하지 말고, 반드시 위 [날짜 참조표]에서 해당하는 날짜를 찾아 입력해야 합니다.**

                 **규칙 A. 날짜가 명시된 경우 (우선순위 높음)**:
                 - 텍스트에 "내일", "모레", "이번주 금요일", "다음주 월요일", "1월 19일까지" 등 날짜 표현이 있다면, **[날짜 참조표]에서 해당 요일의 날짜(YYYY-MM-DD)를 찾아 그대로 적으세요.**
                 - 예: '다음주 월요일'이라고 하면, 표에서 이번 주 일요일 구분선 아래에 있는 첫 번째 월요일 날짜를 찾으세요.
                 - 주말이나 공휴일 여부는 신경 쓰지 말고 표에 있는 날짜 그대로 적으세요.
