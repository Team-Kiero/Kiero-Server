...
public enum ScheduleErrorCode implements BaseCode {
	...
	FIRE_LIT_ALREADY_COMPLETED(HttpStatus.BAD_REQUEST, "오늘의 불 피우기가 이미 완료되었습니다."),
	...
	DATE_NOT_NULLABLE_WHEN_RECURRING_IS_FALSE(HttpStatus.BAD_REQUEST, "반복 일정이 아닐 때, 일정 일자가 입력되어야 합니다."),
	...
	REQUIRED_PARAMS_FOR_RECURRING_SCHEDULE(HttpStatus.BAD_REQUEST, "반복일정을 삭제 또는 수정할 경우, 바디에 isIncludeFollowing, 쿼리 파라미터에 startDate, endDate를 포함해야 합니다."),
	REQUIRED_PARAMS_FOR_NORMAL_SCHEDULE(HttpStatus.BAD_REQUEST, "단일일정을 삭제 또는 수정할 경우, 쿼리 파라미터에 selectedDate를 포함해야 합니다."),
	...
	SCHEDULE_CANNOT_BE_MANIPULATED_WHEN_FIRE_LIT(HttpStatus.BAD_REQUEST, "오늘의 불 피우기가 완료되었을 때, 오늘 단일 일정을 조작할 수 없습니다."),
	SCHEDULE_CANNOT_BE_MANIPULATED_WHEN_AFTER_SCHEDULE_NOT_PENDING(HttpStatus.BAD_REQUEST, "등록하려는 일정의 이후 일정 중 아이가 행위를 진행한 일정이 있다면 등록 불가능합니다."),
